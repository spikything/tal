  ...
  function (Label, TextPageChangeEvent) {
    ...
    var TextPager = Label.extend( /** @lends antie.widgets.TextPager.prototype */ {
      ...
      setPage: function setPage(page) {
        ...
        this._page = page;
        var newScrollTop = -((page - 1) * this.lineHeight * this.lines_in_box);
        this.getCurrentApplication().getDevice().getLogger().log("scroll to page " + this._page + " " + newScrollTop);
        var scope = this;
        var device = scope.getCurrentApplication().getDevice();
        if (scope._moveHandle) {
          device.stopAnimation(scope._moveHandle);
        }
        scope._moveHandle = device.tweenElementStyle({
          el: scope.outputElement,
          to: {
            top: newScrollTop
          },
          skipAnim: false,
