/**
 * @fileOverview Requirejs module containing device modifier for shaka.js media playback
 */
define(
  'antie/devices/mediaplayer/shaka', [
  ...
  function (RuntimeContext, Device, MediaPlayer) {
     ...
     * Use this device modifier if a device implements the shaka.js media playback library.
     * @name antie.devices.mediaplayer.shaka
     * @class
    ...
    var Player = MediaPlayer.extend({
      ...
      loadPlugin: function (cb) {
        var scope = this;
        require(["https://cdn.jsdelivr.net/npm/shaka-player@3.3.0/dist/shaka-player.compiled.min.js"], function (shaka) {
          console.log("shaka.js loaded");
          Shaka = shaka;
          shaka.polyfill.installAll(); // Install built-in polyfills to patch browser incompatibilities.

          shaka.configure("streaming.useNativeHlsOnSafari", false);
          if (shaka.Player.isBrowserSupported()) { // Check to see if the browser supports the basic APIs Shaka needs.
          ...
          console.log("shaka-player failed to load");
        });
      },
      ...
      createMediaElement: function (mediaType, device) {
        ...
        this.shaka = new Shaka.Player(this._mediaElement);
      },
      /**
       * @inheritDoc
       */
      ...
      mediaElementPlay: function () {
        var scope = this;
        this.shaka.load(scope._source).then(function () {
          console.log('The manifest has been loaded');
          scope._mediaElement.play();
        }).catch(function (error) {
          scope._toError("source error")
