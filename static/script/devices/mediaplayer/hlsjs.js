/**
 * @fileOverview Requirejs module containing device modifier for hls.js media playback
 */
define(
  'antie/devices/mediaplayer/hlsjs', [
  ...
  function (RuntimeContext, Device, MediaPlayer) {
     ...
     * Use this device modifier if a device implements the hls.js media playback library.
     * @name antie.devices.mediaplayer.HLSJS
     * @class
    ...
    var Player = MediaPlayer.extend({
      ...
      loadPlugin: function (cb) {
        var scope = this;
        require(["http://cdn.jsdelivr.net/npm/hls.js@0.13.2/dist/hls.min.js"], function (hls) {
          ...
          console.log("HLS.js failed to load");
        });
      },
      ...
      _destroyMediaElement: function _destroyMediaElement() {
        if (this._mediaElement) {
          this.hls.detachMedia(this._mediaElement);
          var scope = this;
      ...
      mediaElementPlay: function () {
        ...
        this.hls.on(Hls.Events.MEDIA_ATTACHED, function () {
          scope.hls.loadSource(scope._source); // load manifest
          scope.hls.on(Hls.Events.MANIFEST_PARSED, function (event, data) {
